...
// The delivery address, written where it is heard. Opened from the comanda's Domicilio card,
// it is a modal with a field big enough to type a real Colombian address into on a tablet
// ("Calle 15 #10-20, barrio San Martín, casa de reja verde"). The pin is NOT picked here:
// saving an address alone lets the backend geocode an approximate one, and /dispatch keeps
// the map picker for correcting it.
...
const emit = defineEmits<{ save: [address: string]; close: [] }>()

// Mirrors the backend's `max_length=255` on address_text.
const MAX = 255
...
// Unlike a kitchen note, an address cannot be cleared — a delivery without one is the
// problem this screen exists to prevent.
const canSave = computed(
  () => trimmed.value.length > 0 && !tooLong.value && !props.saving,
)
  ...
  <Teleport to="body">
    ...
    <div class="pointer-events-none fixed inset-0 z-[60] flex items-end justify-center lg:items-center">
      <aside
        ...
        role="dialog"
        aria-modal="true"
        aria-label="Dirección del domicilio"
      >
        <header class="flex items-start justify-between gap-3 border-b border-line px-5 py-4">
          <div class="min-w-0">
            <p class="eyebrow">Domicilio</p>
            <p class="truncate font-display text-lg font-bold text-ink">
              {{ address ? 'Corregir dirección' : 'Dirección de entrega' }}
            </p>
          </div>
          <button
            type="button"
        ...
        <div class="px-5 py-4">
          <label class="sr-only" for="address-field">Dirección de entrega</label>
          <textarea
            id="address-field"
            ref="field"
            v-model="draft"
          ...
          <div class="mt-2 flex items-center justify-between gap-2">
            <p class="font-mono text-[11px] text-steel-400">
              El pin se ubica solo · se corrige en Domicilios
            </p>
            <p
...
.sheet {
  animation: address-in 0.24s cubic-bezier(0.2, 0.7, 0.2, 1) both;
}
@media (min-width: 1024px) {
  @keyframes address-in-lg {
    from {
      opacity: 0;
